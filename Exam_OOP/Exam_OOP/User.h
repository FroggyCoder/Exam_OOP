...
inline void Admin::Modification()
{
	...
	try
	{
		...
		if (!ifs.is_open())
		{
			gotoxy(25, ++gotx);
			SetColor(Red, Black);
			cout << "Ошибка открытия файла c данными студентов Admin::Modification()";
			SetColor(15, Black);
			ifs.close();
			gotoxy(25, ++gotx);
			system("pause");
		...
		else
		{
			...
			{
				gotoxy(25, ++gotx);
				cout << "Ошибка открытия файла c данными студентов Admin::Modification() path2";
				ifs.close();
				gotoxy(25, ++gotx);
				system("pause");
			}
			...
			else
			{
				...
				if (it != mp.end())
				{
					...
					if (remove(login.c_str()) != 0)
					{
						gotoxy(25, ++gotx);
						SetColor(Red, Black);
						perror("Ошибка удаления файла с логином студента. Введен неверный логин");
						SetColor(15, Black);
						gotoxy(25, ++gotx);
						system("pause");
					}
...
inline void Admin::EditCategories()
{
	...
	try
	{
		...
		else
		{
			...
			{
				gotoxy(25, ++gotx);
				cout << "В системе еще нет созданных разделов или тестов.";
			}
			...
			else
			{
				...
				else
				{
					gotoxy(25, ++gotx);
					SetColor(Red, Black);
					puts("Вы ввели неверный номер раздела для редактирования");
					SetColor(15, Black);
				}
			}
		}
...
inline void Admin::AddTestsName()
{
	...
	try
	{
		...
		else
		{
			...
			{
				gotoxy(25, ++gotx);
				cout << "В системе нет созданных резделов для тестов.";
				gotoxy(25, ++gotx);
				system("pause");
			}
		}
	...
	try
	{
		...
		else
		{
			...
			if (mp.size())
			{
				...
				{
					gotoxy(25, ++gotx);
					cout << "Вы ввели неверный номер раздела. Есть вероятность, что разделов еще нет.";
				}
			}
